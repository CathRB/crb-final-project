...
const Plant = ({index, plant, setGardenId, setOpen, setErrorMessage, setPlantIndex}) => {
        ...
        <PlantBox key={plant.myGardenId}>
         ...
         { Math.floor(( Date.parse(date) - Date.parse(plant.lastFertilizing)) / 86400000) < plant.fertilizerFrequency || !plant.lastFertilizing || !plant.fertilizerFrequency ?(
          ...
          <BottomBox>
            ...
            <SettingsButton onClick={()=>{handleClickOpen(); setGardenId(plant.myGardenId); setPlantIndex(index)}}>Change my personal settings</SettingsButton>
         ): (<SettingsButton  onClick={()=>{handleClickOpen(); setGardenId(plant.myGardenId); setPlantIndex(index)}}>Add my personal settings</SettingsButton>)}
          ...
          <button style={{ display: yesNoButton }} onClick={(event) => removePlant(event, plantToRemove, setErrorMessage, removeFromMyGarden, navigate)}>Yes</button>
